	...
	if(nvml)
	{
		for(int i=0; i < nvml_device_count; i++)
		{
			int ret = nvmlDeviceGetTemperature(nvml_devices[i], 0,
				&nvml_gputemp[i]);
			if(ret)log_debug("nvmlDeviceGetTemperature(%d) = %d", i, ret);
//			log_debug("Temperature[%d] is %d", i, temp);
		}
	...
	if(adl)
	{
		...
		case 6:
			...
			adl_gputemp[i] /= 1000;
			if(ret)log_debug("ADL_Overdrive6_Temperature_Get(%d) = %d", i, ret);
			break;
		case 5:
			...
			if(ret)log_debug("ADL_Overdrive5_Temperature_Get(%d) = %d", i, ret);
...
#define EvilMacro(L, T, N, NS) \
	...
	if(!N)log_error("GetProcAddress(\"N\") failed")

#else

#define EvilMacro(L, T, N, NS) \
	*(void **)(&N) = dlsym(L, NS); \
	if(!N)log_error("dlsym(\"N\") failed")
...
void gpuinfo_init(void)
{
	...
	if(nvml)
	{
		...
		int ret = nvmlInit();
		if(ret)log_debug("nvmlInit = %d", ret);

		ret = nvmlDeviceGetCount(&nvml_device_count);
		if(ret)log_debug("nvmlDeviceGetCount = %d", ret);
		...
		for(int i=0; i < nvml_device_count; i++)
		{
			...
			if(ret)log_debug("nvmlDeviceGetHandleByIndex(%d) = %d", i, ret);
		}
		log_debug("NVML ");
	...
	if(adl)
	{
		...
		if(ret)log_debug("ADL_Main_Control_Create = %d", ret);
		ret = ADL_Adapter_NumberOfAdapters_Get(&adl_device_count);
		if(ret)log_debug("ADL_Adapter_NumberOfAdapters_Get = %d", ret);
		...
		for(int i=0; i < adl_device_count; i++)
		{
			...
			if(ret)log_debug("ADL_Overdrive_Caps(%d) = %d", i, ret);
			if(5 == adl_odver[i])
			{
				int j;
				for(j=0; j<10; j++)
...
void gpuinfo_end(void)
{
	...
	if(adl)	// for AMD GPU temperature
	{
		...
		if(ret)log_debug("ADL_Main_Control_Destroy = %d", ret);
